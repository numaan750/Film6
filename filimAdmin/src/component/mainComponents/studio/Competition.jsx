...
const Competition = ({ competate, setCompetate, competateImage, setCompetateImage, studioId, oldCompetateImage, setOldCompetateImage, sectionName }) => {

  const onChangeHandler = (event) => {
    const name = event.target.name;
    const value = event.target.value;
    setCompetate((data) => ({ ...data, [name]: value }));
  };

  console.log(competate, "toplist2");
  return (
    <div>
      <div className="p-4 border mt-12">
        ...
        <form>
          ...
          <div className="rounded-md border border-indigo-500 bg-gray-50 p-4 shadow-md w-36">
            ...
            <input
              // onChange={(e) => setCompetateImage(e.target.files[0])}
              onChange={(e) => {
  ...
  <div className="mt-4">
    ...
    <img
      src={competateImage instanceof File ? URL.createObjectURL(competateImage) : competateImage}
      alt="Preview"
      className="w-36 h-auto"
    />
  </div>
)}
{oldCompetateImage && oldCompetateImage.length > 0 && (
  <div className="mt-4">
    <h2 className="text-sm font-semibold text-gray-500 mb-1">OLD IMAGES</h2>
    <div className="flex gap-2 flex-wrap">
      {[...oldCompetateImage].reverse().map((url, index) => (
          ...
          <div className="mt-8">
            ...
            <div className="mb-4 mt-4">
              ...
              <p className="text-sm text-gray-500 mt-1">
                {competate.description?.trim() === ""
                  ? 0
                  : competate.description?.trim().split(/\s+/).length || 0}
                /60 words
              </p>
            </div>
            <div className="mb-4 mt-4">
              ...
              />
              <p className="text-sm text-gray-500 mt-1">
                {competate.description2?.trim() === ""
                  ? 0
                  : competate.description2?.trim().split(/\s+/).length || 0}
                /60 words
              </p>
            </div>
            <div className="mb-4">
